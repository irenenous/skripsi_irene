...
<?php
	...
	$subject = $_GET['subjek']; }
    $updatestat = "UPDATE pesan SET status = 'READ' Where id_eo = '$ideo' AND subjek ='$subject' AND sender = 'KLIEN'";
    $simpan = mysqli_query($koneksi, $updatestat);
    if ($simpan) {
    $query1="SELECT * FROM pesan INNER JOIN user ON pesan.id_user = user.id_user where subjek = '".$subject."' AND id_eo = ".$ideo;
	$tampil1 = mysqli_query($koneksi,$query1);
    $select1 = mysqli_fetch_array($tampil1);     
            $idmsg	    = $select1['id_pesan'];
			$date	    = $select1['tgl_pesan'];
            $idklien    = $select1['id_user'];
			$klien      = $select1['nama_user']; 
            $fotoklien  = $select1['foto_user'];
      ...
      <div class="page-content d-flex align-items-stretch"> 
        ...
        <div class="content-inner">
          ...
          <div class="breadcrumb-holder container-fluid">
            ...
            <li class="breadcrumb-item"><a href="inbox-eo.php">Inbox</a></li>
...
<div class="row">
		<div class="col-lg-12">
			...
			<div class="message-vendor-img-detail">
				<img src="../user/<?php echo $fotoklien ?>" class="img-vendor-message">
			</div>
			<div class="section-message-detail">
				<div class="message-detail-name">
					<span class="vendor-name"><?php echo $klien ?></span>
				</div>
				<div class="message-detail-name">
...
<?php
    ...
    $query2="SELECT * FROM pesan INNER JOIN user ON pesan.id_user = user.id_user where subjek = '".$subject."'";
...
<script>
    function send_message_function() {
                ...
                $.ajax({                                     
                    ...
                    timeout: 600000,
                    data: {
                        "id_user" : "<?php echo( ($_SESSION['is_eo']) ? $idklien : $ideo); ?>",
                        "id_eo": "<?php echo(($_SESSION['is_eo']) ? $ideo : $idklien); ?>",
